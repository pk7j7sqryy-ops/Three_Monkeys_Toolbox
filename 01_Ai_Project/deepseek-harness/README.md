...
这个目录收集我为 [DeepSeek Harness (DSH)](https://github.com/deepseek-ai/deepseek-harness) 写的动态 Cordis 插件。
...
| 目录 | 说明 | 类型 |
|------|------|------|
| [token-pet](./token-pet) | Token 泡泡：右上角卡通用量小部件（布布玩偶 + 占用/累计/构成） | 动态插件（Client） |
...
- 每个插件一个子目录，含 `client.js`（或 `host.js`）源码 + 自己的 `README.md`。
